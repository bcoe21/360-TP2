...
public class ControllerUserLogin {
	...
	*/
	...
	protected static void doLogin(Stage ts) {
    	...
    	String otp = theDatabase.getCurrentOneTimePassword(); // ***MODIFIED***
    	boolean mustChangePassword = theDatabase.getCurrentChangePassword(); // ***MODIFIED***
    	
    	// ***MODIFIED***
    	boolean normalLogin = password.equals(actualPassword);
    	boolean otpLogin = (otp != null && password.equals(otp));
    	
    	// if (mustChangePassword == true), do NOT allow the normal password
    	// force user to use the one-time password
    	if (mustChangePassword && !otpLogin) {
    	    ViewUserLogin.alertUsernamePasswordError.setContentText(
    	        "A one-time password was issued for this account. Please log in using the one-time password."
    	    );
    	    ViewUserLogin.alertUsernamePasswordError.showAndWait();
    	    return;
    	}
    	...
    	// ***MODIFIED***
    	// regular invalid credentials check
    	if (!normalLogin && !otpLogin) {
    	    ViewUserLogin.alertUsernamePasswordError.setContentText(
    	            "Incorrect username/password. Try again!");
    	    ViewUserLogin.alertUsernamePasswordError.showAndWait();
    	    return;
    	...
    	// ***MODIFIED***
    	// after OTP is used, we force them to the update password page
    	if (otpLogin) {
    	    applicationMain.FoundationsMain.forceReloginAfterSetup = true;
