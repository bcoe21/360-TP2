...
public class ControllerAddRemoveRoles {
	...
	private static void setupSelectedUser() {
		...
		if (!theDatabase.getCurrentNewRole1())
			ViewAddRemoveRoles.addList.add("Student"); // ***MODIFIED***
		if (!theDatabase.getCurrentNewRole2())
			ViewAddRemoveRoles.addList.add("Staff"); // ***MODIFIED***
		...
		if (theDatabase.getCurrentNewRole1())
			ViewAddRemoveRoles.removeList.add("Student"); // ***MODIFIED***
		if (theDatabase.getCurrentNewRole2())
			ViewAddRemoveRoles.removeList.add("Staff"); // ***MODIFIED***
		...
		if (theDatabase.getCurrentNewRole1()) {
			if (notTheFirst)
				theCurrentRoles += ", Student"; // ***MODIFIED***
			else {
				theCurrentRoles += "Student"; // ***MODIFIED***
				notTheFirst = true;
			}
		...
		if (theDatabase.getCurrentNewRole2()) {
			if (notTheFirst)
				theCurrentRoles += ", Staff"; // ***MODIFIED***
			else {
				theCurrentRoles += "Staff"; // ***MODIFIED***
				notTheFirst = true;
			}
	...
	protected static void performAddRole() {
		...
		if (ViewAddRemoveRoles.theAddRole.compareTo("<Select a role>") != 0) {
			...
			if (theDatabase.updateUserRole(ViewAddRemoveRoles.theUser.getUserName(),
			        ViewAddRemoveRoles.theSelectedUser,
	...
	// ***MODIFIED***
	private static void showRoleUpdateError() {
	    Alert alert = new Alert(Alert.AlertType.ERROR);
	    alert.setTitle("Role Update Failed");
	    alert.setHeaderText("Unable to update role");

	    String msg = theDatabase.getLastErrorMessage();
	    if (msg == null || msg.trim().isEmpty()) {
	        msg = "The role update could not be completed.";
	    }
