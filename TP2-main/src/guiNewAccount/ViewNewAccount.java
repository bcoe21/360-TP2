...
public class ViewNewAccount {
	...
	*/
	...
	public static void displayNewAccount(Stage ps, String ic) {
		...
		// If there is an issue with the invitation code, display a
		// dialog box saying that are when it it
		// acknowledged, return so the proper code can be entered
		if (theRole.length() == 0) {
			// ***MODIFIED***
			// check to see whether the invite is expire vs invalid
		    if (theDatabase.isInvitationExpired(theInvitationCode)) {
		        alertInvitationCodeIsInvalid.setTitle("Invitation Expired");
		        alertInvitationCodeIsInvalid.setHeaderText("This invitation code has expired.");
		        alertInvitationCodeIsInvalid.setContentText("Ask an admin to send a new invitation code.");
		    } else {
		        ...
		        alertInvitationCodeIsInvalid.setContentText("Please enter a valid code.");
		    }
