...
public class ControllerUserUpdate {
	...
	**********************************************************************************************/
	...
	protected static void goToUserHomePage(Stage theStage, User theUser) {
		...
		// ***MODIFIED***
		// here we force re-login after first-time account setup
		if (applicationMain.FoundationsMain.forceReloginAfterSetup) {
		    applicationMain.FoundationsMain.forceReloginAfterSetup = false; // clearing the flag

		    Alert alert = new Alert(AlertType.INFORMATION);
		    alert.setTitle("Login Required");
		    alert.setHeaderText(null);
		    alert.setContentText("Your account setup is complete. Please log in again to continue.");
		    alert.showAndWait();
