...
public class ViewUserUpdate {
	...
	// ***MODIFIED***
	private static javafx.scene.control.Alert alertMustChangePassword =
	...
	private static TextInputDialog dialogUpdatePassword; // ***MODIFIED***
	...
	public static void displayUserUpdate(Stage ps, User user) {
	    ...
	    // ***MODIFIED***
	    // disable home page access during OTP reset (changePassword = TRUE)
	    button_ProceedToUserHomePage.setDisable(mustChangePasswordNow());
		theStage.show();
	}
	...
	private ViewUserUpdate() {
		...
		dialogUpdatePassword = new TextInputDialog(""); // ***MODIFIED***



		// Establish the label for each of the dialogs.
		dialogUpdatePassword.setTitle("Update Password"); // ***MODIFIED***
		dialogUpdatePassword.setHeaderText("Update your Password"); // ***MODIFIED***
		...
		// ***MODIFIED***
		alertMustChangePassword.setTitle("Password Required");
		alertMustChangePassword.setHeaderText("Action Required");
		alertMustChangePassword.setContentText(
        ...
        button_UpdatePassword.setOnAction((_) -> {
            // check to see if redirected from OTP
            boolean cameFromOtp = applicationMain.FoundationsMain.forceReloginAfterSetup;

            result = dialogUpdatePassword.showAndWait();
            result.ifPresent(newPassword -> {
                if (newPassword == null || newPassword.trim().length() < 1) return;
                ...
                // after changing password, proceed to home page
                button_ProceedToUserHomePage.setDisable(false);
                
                // if user came from OTP, force re-login after changing password
                if (cameFromOtp) {
                    applicationMain.FoundationsMain.forceReloginAfterSetup = false;
        ...
        button_ProceedToUserHomePage.setOnAction((_) -> {
            ...
            // blocks home page access during OTP reset (changePassword = TRUE in DB)
            if (mustChangePasswordNow()) {
                alertMustChangePassword.showAndWait();
                return;
            }
	...
	// ***MODIFIED***
	// true when user must change password (from OTP)
	private static boolean mustChangePasswordNow() {
	    theDatabase.getUserAccountDetails(theUser.getUserName());
	    return theDatabase.getCurrentChangePassword();
	}
